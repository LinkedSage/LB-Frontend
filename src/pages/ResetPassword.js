    ...
    if (phoneEmail) {
      ...
      if (isNumber(phoneEmail))
        result = await Axios.post(
          `${process.env.REACT_APP_API_URL}/user/request-reset-password`,
          { phone: phoneEmail }
        );
      else
        result = await Axios.post(
      ...
      notification("warning", "Please enter your Phone No. or Email");
    }
  }
                  ...
                  {otpFlag ? (
                    ...
                    <div className="col-md-4 pt-2">
                      <button
                        onClick={() => sendOtpFun()}
                        type="button"
                        className="w-50 text-white  pb-2 pt-2 glow-on-hover"
                      >
                        Get OTP
                      </button>
                    </div>
