...
export default function Home() {
  ...
  function setDefaultValues() {
    ...
    Object.keys(currentUser).forEach((key) => {
      if (
        key === "nid_front_image_url" ||
        key === "nid_back_image_url" ||
        key === "account_statement_pdf_url" ||
        key === "payslip_pdf_url" ||
        key === "image_url"
      ) {
    ...
    if (
      tempUser &&
      tempUser.employeement_information &&
      tempUser.employeement_information.company_name
    )
      setOrganization({
        label: tempUser.employeement_information.company_name,
        value: tempUser.employeement_information.company_name,
      });
  ...
  function submitNIDFrontBack() {
    var fd = new FormData();
    if (sendNIDFront) fd.append("nid_front_image_url", sendNIDFront);
    if (sendNIDBack) fd.append("nid_back_image_url", sendNIDBack);
  ...
  function submitFinancial() {
    var fd = new FormData();
    if (sendFile) fd.append("account_statement_pdf_url", sendFile);
    if (sendFilePaySlip) fd.append("payslip_pdf_url", sendFilePaySlip);
    console.log("nnnnnnnnnnn", fd);
    updateUserProfile(fd);
  }
